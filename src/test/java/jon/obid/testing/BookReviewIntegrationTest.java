 ...
 *
 *  Spring Boot 2.4.x notes:
 *   - @WebMvcTest includes Spring Security auto-configuration automatically — no
 *     @ImportAutoConfiguration needed, unlike Boot 4.x.
 *   - @MockBean (org.springframework.boot.test.mock.mockito) is the standard in Boot 2.x.
 *     @MockitoBean was not yet available (introduced in Spring Framework 6.2 / Boot 3.4).
 ...
 *   - @DynamicPropertySource available since Spring Framework 5.2.5 (Boot 2.2.6+).
 *
...
class BookReviewIntegrationTest {
  ...
  // Boot 2.x: @MockBean from org.springframework.boot.test.mock.mockito.
  @MockBean
  ...
  void shouldReturnAggregatedStatisticsForAuthenticatedUser() throws Exception {
    ...
    // Boot 2.4: the PostgreSQL JDBC driver returns ROUND(AVG(...), 2) as BigDecimal with
    // scale 0 when the result is a whole number (e.g. AVG(5,3)=4). Jackson 2.x then
    // serializes BigDecimal(4) as the integer 4, which JsonPath parses as Integer — not Double.
    // Using is(4) matches the Integer directly. Boot 3.5+ files use closeTo(4.0, 0.01).
    mockMvc
