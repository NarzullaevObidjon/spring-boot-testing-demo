 ...
 *
 *  @RestClientTest auto-configures MockRestServiceServer — a Spring-managed interceptor
 *  that replaces the RestTemplate's HTTP transport.  No actual network activity occurs.
 *
 *  How it works:
 *   1. @RestClientTest loads only the target client bean and RestTemplateAutoConfiguration.
 *   2. MockRestServiceServer is registered as a bean and wires into the RestTemplate.
 *   3. We declare expectations with .expect(...) and pre-configure responses.
 *   4. After the test, MockRestServiceServer verifies all expectations were satisfied.
 *
 *  Spring Boot 3.x vs 4.x:
 *   RestTemplateAutoConfiguration registers RestTemplateBuilder, not RestTemplate.
 *   The @TestConfiguration inner class builds the RestTemplate so MockRestServiceServer
 *   can intercept it — this pattern applies to both Boot 3.x and Boot 4.x.
...
class OpenLibraryRestTemplateApiClientTest {
  ...
  // RestTemplateAutoConfiguration only registers RestTemplateBuilder, not RestTemplate.
  // Provide a RestTemplate bean explicitly so MockRestServiceServer can wire into it.
  @TestConfiguration
